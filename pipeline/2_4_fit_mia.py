...
def main():
    parser = argparse.ArgumentParser(description='Load datasets from given links and fit MIA models')
    ...
    for row in tqdm(catalog.itertuples(), desc="Models fitted:"):
        ...
        model = MIA().fit(train_data.drop(columns=['Y']), train_data['Y'])
        train_info.append({
            "model_path": output / f"{row.train_path.split('/')[-1][:-4]}.pkl"
        })
